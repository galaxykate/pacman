...
public class Executor {
	...
	public static void main(String[] args) {
		...
		int numTrials = 10000;
		// exec.runExperiment(new StarterPacMan(),new
		// Legacy2TheReckoning(),numTrials);
		...
		// Legacy2TheReckoning(),visual,bRunUnitTests);
		exec.runGameTimed(new KatesPacMan(), new Legacy2TheReckoning(), visual,
				bRunUnitTests);
	...
	public void runGameTimed(Controller<MOVE> pacManController,
		...
		while (!game.gameOver()) {
			pacManController.update(game.copy(), System.currentTimeMillis()
					+ DELAY);
			ghostController.update(game.copy(), System.currentTimeMillis()
					+ DELAY);
			...
			game.advanceGame(pacManController.getMove(),
			...
			if (bRunUnitTests) {
				eval.runUnitTests(game, pacManController, ghostController,
						runActionsAndConditionsTests, runDecisionTreeTests,
						runFSMTests, runHFSMTests);
	...
	public void runGameTimedSpeedOptimised(Controller<MOVE> pacManController,
		...
		while (!game.gameOver()) {
			...
			try {
				...
				if (fixedTime)
					Thread.sleep(((DELAY / INTERVAL_WAIT) - waited)
							* INTERVAL_WAIT);
	...
	public void runGameTimedRecorded(Controller<MOVE> pacManController,
		...
		if (visual) {
			...
			if (pacManController instanceof HumanController)
				gv.getFrame()
						.addKeyListener(
								((HumanController) pacManController)
										.getKeyboardInput());
		}
